...
/**
 * The TimeSlotActivity class extends ListActivity and is responsible for
 * filling in the individual TimeSlot tabs with the Room, Presenter and
...
public class TimeSlotActivity extends ListActivity {
	...
	/**
	 * timeSlotName is the name of the timeSlot (i.e. Slot 1, Slot 2, etc).
	 */
	private String timeSlotName;

	/**
	 * Called when the activity is first created. Gets the time slot name and
	 * loads the presentations during that time slot. This method then populates
	 * the ListView with the presentations.
	 * */
	public void onCreate(Bundle savedInstanceState) {
		...
		ListAdapter la = new SimpleAdapter(this, list, R.layout.list_item,
				new String[] { "line1" }, new int[] { R.id.text1 });

		// The following code adds a 'drop down' animation for the items in the
		// ListView.
		...
		/**
		 * On Click, lookup the presentation title and parse out the string for
		 * the list item to properly compose the embedded HTML files name for
		 * display in the PresentationActivity.
		 */
		lv.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> parent, View view,
				...
				s4 = s3[1].split("\n");
				// Parse out any special characters (space,:, and \) in the
				// presentation_Desc
