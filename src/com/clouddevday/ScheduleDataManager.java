...
/**
 * The ScheduleDataManager class is used to read in the Schedule Data at
 * application start. It has member variables and methods to read the data from
 * either a remote URL, a local data file, or a resource string.
 * 
 * @author Don Ward
 */
public class ScheduleDataManager {
	 ...
	 * checks for remote data. If it exists, then it stores the raw data in the
	 * rawRemoteData member variable and also parses the data with an XML parser
	 * 
	 * boolean dataExists
	 */
	...
	public int getLocalVersionNumber() {
		try {
			FileInputStream fis = MyApp.getContext().openFileInput(
					localDataFileName);
	...
	/**
	 * localDataFileExists - return a boolean indicating whether the Local File
	...
	/**
	 * readLocalResourceStringData - reads the schedule data from the local
	 * resource string
	 * 
	 * @return boolean success
	 */
	public void readLocalResourceStringData() {
		timeRoomsPresenters = MyApp.getContext().getResources()
				.getStringArray(R.array.time_room_presenter_array);
	}

	/**
	 * storeRemoteDatatoLocalDataFile - stores remote data to the Local Data
	 * file which is an Internal File private to this application
	 * 
	 * @return boolean success
	 */
	public boolean storeRemoteDatatoDataFile() {
		try {
			FileOutputStream fos = MyApp.getContext().openFileOutput(
					localDataFileName, MyApp.getContext().MODE_PRIVATE);
